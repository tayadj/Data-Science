...
def get_order(n_samples):
    try:
        with open('./data/' + str(n_samples) + '.txt') as fp:
            line = fp.readline()
...
def hinge_loss_single(feature_vector, label, theta, theta_0):
    ...
    return max(0, 1 - label * (feature_vector @ theta + theta_0))
...
def average_perceptron(feature_matrix, labels, T):
    ...
    theta = np.zeros(feature_matrix.shape[1])
    theta_0 = 0
    history_theta = np.zeros(feature_matrix.shape[1])
    history_theta_0 = 0
    ...
    for t in range(T):
        for i in get_order(nsamples):
            theta, theta_0 = perceptron_single_step_update(feature_matrix[i], labels[i], theta, theta_0)
            history_theta += theta
            history_theta_0 += theta_0
   
    return history_theta / (T * nsamples), history_theta_0 / (T * nsamples)
...
def classify(feature_matrix, theta, theta_0):
    ...
    """
    return np.array([+1 if np.dot(theta, feature_vector) + theta_0 > 0 else -1 for feature_vector in feature_matrix])
